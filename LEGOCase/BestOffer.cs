...
public class BestOffer
{
    ...
    public void FindBestOffer(MaterialList materials, VendorList vendorList)
    {
        if (materials.Materials != null)
        {
            var resultByName = materials.Materials
                .Where(m => m.Name != null && m.Name.Equals("Polymethyl Methacrylate (PMMA)"))
                .ToList();
    ...
    {
        if (material.TempUnit != null && material.TempUnit.Equals("F"))
        {
            material.TempUnit = "C";
    ...
    {
        if (vendors.Vendors != null && vendors.Vendors.Find(x => x.Id == id)!.EcoFriendly)
        {
            return true;
        }
